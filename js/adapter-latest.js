...
(function() {
    var pluginObj;
    var browserDetails = { browser: 'unknown' }, utils, logging;
    var ieShim, chromeShim, edgeShim, firefoxShim, safariShim;
    ...
    if (isInternetExplorer) {
        //TODO(dmi): add support for 'require'
        var tag_hdr = document.getElementsByTagName("head")[0];
        if (tag_hdr) {
            browserDetails = { browser: 'ie' };
            var tag_script = document.createElement('script');
            tag_script.setAttribute('type', 'text/javascript');
            tag_script.setAttribute('src', "./ie/ie_shim.js");
        ...
        utils = require('./utils');
        logging = utils.log;
        browserDetails = utils.browserDetails;
        ...
        // Browser shims.
        chromeShim = require('./chrome/chrome_shim') || null;
    ...
    switch (browserDetails.browser) {
        case 'ie':
            {
                // FIXME(dmi): move in seperate file like what is done for chrome and other browsers
                pluginObj = document.createElement('object');
                pluginObj.setAttribute('classid', 'CLSID:F8CFFADD-7ED1-46E9-BEFD-DE7DE1092074');
                pluginObj.setAttribute('id', 'rtcPluginId');
                document.body.appendChild(pluginObj);
                pluginObj.setAttribute('width', '0');
                pluginObj.setAttribute('height', '0');
                console.log("Plugin version: " + pluginObj.versionName + ", adapter version: 1.1.2");

                // === getWebRTCPlugin === //
                getWebRTCPlugin = window.getWebRTCPlugin = function() {
                    return pluginObj.isWebRtcPlugin ? pluginObj : null;
                }

                // === getUserMedia === //
                navigator.mediaDevices = pluginObj.mediaDevices;

                // === RTCPeerConnection === //
                RTCPeerConnection = window.RTCPeerConnection = function(configuration, constraints) {
                    return pluginObj.createPeerConnection(configuration, constraints);
                }

                // === RTCIceCandidate === //
                RTCIceCandidate = window.RTCIceCandidate = function(RTCIceCandidateInit) {
                    return pluginObj.createIceCandidate(RTCIceCandidateInit);
                }

                // === MediaStream === //
                MediaStream = window.MediaStream = function() {
                    return pluginObj.createMediaStream();
                }

                // === Others === //
                getScreens = window.getScreens = function() {
                    return pluginObj.getScreens();
                }
                getWindows = window.getWindows = function() {
                    return pluginObj.getWindows();
                }

                // === attachMediaStream === //
                attachMediaStream = window.attachMediaStream = function(element, stream) {
                    ...
                    if (element.isWebRtcPlugin) {
                        ...
                        if (!element.pluginObj && stream) {
                            ...
                            _pluginObj.setAttribute('classid', 'CLSID:F8CFFADD-7ED1-46E9-BEFD-DE7DE1092074');
                ...
                // === webrtcloaded === //
                if (pluginObj.isWebRtcPlugin) {
                    var webrtcloaded;
                    if (document.createEvent) {
                        webrtcloaded = document.createEvent("HTMLEvents");
                        webrtcloaded.initEvent("webrtcloaded", true, true);
                    } else {
                        webrtcloaded = document.createEventObject();
                        webrtcloaded.eventType = "webrtcloaded";
                    }
                    webrtcloaded.eventName = "webrtcloaded";
                    if (document.createEvent) {
                        document.dispatchEvent(webrtcloaded);
                    } else {
                        document.fireEvent("on" + webrtcloaded.eventType, webrtcloaded);
                    }
                }
